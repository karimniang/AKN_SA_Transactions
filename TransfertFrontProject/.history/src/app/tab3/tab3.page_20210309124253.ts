...
export class Tab3Page {
  ...
  async showFrais(){
    this.fraisToShow = await this.caluclFrais(+this.montant);
    console.log(this.fraisToShow);
    this.alertController.create({
      header:"Calcul de frais",
      message: "Pour un montant de "+this.montant+" le frais est de "+this.fraisToShow+" !!",
      buttons:["Retour"],
    }).then(
      res=>{
        res.present();
