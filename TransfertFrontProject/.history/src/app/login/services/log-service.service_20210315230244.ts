...
export class LogServiceService {
  ...
  httpHeader = {
    headers: new HttpHeaders({ 'Content-Type': 'application/json', 'access-control-allow-origin' :' http://localhost:8100'  })
  };

   login(username, password) {
      this.httpClient.post(this.urlApi, { username, password }).subscribe(
       (re)=>{
         console.log(re);
         return re
       },err=>{
  ...
  async saveToken(username,password) {
    const result: any =  this.login(username,password);
    //console.log('ok');
