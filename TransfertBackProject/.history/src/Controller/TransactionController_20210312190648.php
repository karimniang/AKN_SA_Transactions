...
class TransactionController extends AbstractController
{
     ...
     * @return JsonResponse
     */
    public function makeDepot(ValidatorInterface $validator,Request $request,TransactionRepository $repoTransaction, AddUserService $addUserService,EntityManagerInterface $manager, SerializerInterface $serializer): JsonResponse
    {
        //dd();
        ...
        $transactionDone = $serializer->serialize($essai,"json",["groups"=>["transaction:done"]]);
     ...
     * @return JsonResponse
     */
    public function makeRetrait(AddUserService $addUserService, Request $request, EntityManagerInterface $manager, TransactionRepository $repository, ClientRepository $repoClient, SerializerInterface $serializer): JsonResponse
    {
        $requestTab = json_decode($request->getContent(),true);
        ...
        $receveur = $addUserService->buildClient($requestTab['receveur']);
        dd($receveur==$transaction->getClientDepot());
        //$clientDepot = $repoClient->findOneBy(["telephone"=>$requestTab["receveur"]["telephone"]]);
        if ($$requestTab["receveur"]["numero_cni"] != $transaction->getClientDepot()->getNumeroCni()){
